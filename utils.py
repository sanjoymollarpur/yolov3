...
def non_max_suppression(bboxes, iou_threshold, threshold, box_format="midpoint"):
    ...
    while bboxes:
        ...
        if len(bboxes_after_nms)>0:
            break
    #print("hello-utils-11-1", len(bboxes_after_nms))
    # if(len(bboxes)==0 and chosen_box1!=0):
    #     bboxes_after_nms.append(chosen_box1)
...
def mean_average_precision(
    ...
    return sum(average_precisions) / len(average_precisions), s1, f1
...
def plot_image(image, boxes, k):
    ...
    for box in boxes:
        ...
        t = class_labels[int(class_pred)]+"-0.0005"
        ax.add_patch(rect)
        plt.text(
            upper_left_x * width,
            upper_left_y * height,
